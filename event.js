...
// URL to fetch the event with expansions
...
fetch(url, {
  ...
  .then(data => {
    ...
    if (data.logo && data.logo.url) {
        ...
        eventImage.src = data.logo.url;
        eventImage.alt = data.name.text;
        eventImage.style.maxWidth = '100%'; // Optional: to make the image responsive
